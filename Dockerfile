...
RUN apt-get update && apt-get install -y build-essential libasound2-dev libpulse-dev git libavahi-compat-libdnssd-dev
WORKDIR /src
RUN git clone https://github.com/librespot-org/librespot.git librespot
WORKDIR /src/librespot
RUN cargo fetch 
RUN cargo build --release --no-default-features --features pulseaudio-backend,with-dns-sd
...
RUN apt-get update && apt-get install -y libpulse0 ca-certificates pamixer libavahi-compat-libdnssd1 && rm -rf /var/lib/apt/lists/* && apt-get clean
